    ...
    <>
      ...
      <div>
        {racesData.map((race: RaceProps) => {
          return <RaceButton key={race.id} race={race} onClick={() => handleButtonRaceClick(race)} />;
        })}
      </div>
      {isSelected ? <button onClick={() => buttonSubmitClick(race as RaceProps, setStage)}>Gooo !</button> : null}
    </>
  );
}
